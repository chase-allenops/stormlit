...
events_by_year = df.groupby('event_year')['total_event_count'].sum().reset_index()
events_by_year_fig = px.line(events_by_year, x='event_year', y='total_event_count', title='Severe Storm Events by Year', labels={'event_year': '', 'total_event_count': 'Total Events'})
st.plotly_chart(events_by_year_fig)

# deaths by year chart
deaths_by_year = df.groupby('event_year')['sum_deaths_direct'].sum().reset_index()
...
damages_by_year = df.groupby('event_year').agg({
...
filtered_df = df[df['event_year'] == str(selected_year)]
